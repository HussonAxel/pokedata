...
import { motion, AnimatePresence, LayoutGroup, MotionConfig, type Transition } from "motion/react";
...
export interface MenuItemOption {
  ...
  badge?: string;
  onClick?: () => void;
  /** Child items rendered in an expandable submenu. */
...
type MenuState = Record<string, Record<string, boolean>>;

const getItemStateKey = (itemPath: string[]) => JSON.stringify(itemPath);

const createInitialMenuState = (tabs: NavTabItem[]): MenuState => {
  const initial: MenuState = {};

  const addItems = (
    tabState: Record<string, boolean>,
    items: MenuItemOption[],
    parentPath: string[] = [],
  ) => {
    items.forEach((item) => {
      const itemPath = [...parentPath, item.id];
      tabState[getItemStateKey(itemPath)] = item.enabled ?? false;
      addItems(tabState, item.submenu ?? [], itemPath);
    });
  };

  tabs.forEach((tab) => {
    const tabState: Record<string, boolean> = {};
    addItems(tabState, tab.menuItems);
    initial[tab.id] = tabState;
  });
...
const solidIcons = {
  ...
  arrowRight: (
    <svg
      ...
      viewBox="0 0 24 24"
      fill="currentColor"
      className="size-3.5 opacity-40 transition-[transform,opacity] group-hover:translate-x-0.5 group-hover:opacity-100"
    >
  ...
  },
  {
    ...
    menuItems: [
      ...
      },
      {
        ...
        sublabel: "Customize your workspace",
        icon: solidIcons.display,
        submenu: [
          {
            id: "theme",
            label: "Theme",
            sublabel: "Choose a color mode",
            submenu: [
              {
                id: "system-theme",
          ...
          },
          {
            ...
            sublabel: "Display collection details",
            enabled: true,
            type: "toggle",
          },
        ],
...
export interface FloatingMenuItemProps {
  ...
  showIcons: boolean;
  isChecked: boolean;
  getItemChecked: (itemPath: string[]) => boolean;
  onToggle: (itemPath: string[], itemId: string) => void;
...
export function FloatingMenuItem({
  ...
  const isInteractive = hasSubmenu || item.type === "action";
  ...
  return (
    <div className="w-full">
      <motion.div
        ...
        aria-label={isInteractive ? item.label : undefined}
        aria-expanded={hasSubmenu ? isSubmenuOpen : undefined}
        aria-haspopup={hasSubmenu ? "menu" : undefined}
        aria-controls={hasSubmenu ? submenuId : undefined}
        className="group flex w-full cursor-pointer items-center justify-between rounded-[14px] px-2.5 py-2 transition-colors hover:bg-muted dark:hover:bg-muted/50"
      >
        <div className="flex min-w-0 flex-1 items-center gap-2.5">
          {showIcons && item.icon && (
            <div className="flex size-7 shrink-0 items-center justify-center rounded-xl bg-muted text-muted-foreground transition-colors group-hover:bg-border group-hover:text-foreground dark:bg-muted/60 dark:text-muted-foreground dark:group-hover:bg-muted dark:group-hover:text-foreground">
              {item.icon}
            </div>
          )}
          <div className="flex min-w-0 flex-1 flex-col">
            <div className="flex items-center gap-1.5">
              <span className="truncate text-xs font-semibold tracking-tight text-secondary-foreground group-hover:text-foreground dark:text-muted dark:group-hover:text-white">
                {item.label}
              </span>
              {item.badge && (
                <span className="rounded-md bg-muted px-1.5 py-0.5 font-mono text-[9px] font-bold text-muted-foreground dark:bg-muted dark:text-muted-foreground">
                  {item.badge}
                </span>
              )}
            </div>
            {item.sublabel && (
              <span className="truncate text-[10px] text-muted-foreground group-hover:text-secondary-foreground dark:text-muted-foreground dark:group-hover:text-foreground">
                {item.sublabel}
              </span>
            )}
          </div>
        ...
        {hasSubmenu ? (
          <motion.span
            ...
            className="flex items-center pl-2 text-muted-foreground group-hover:text-secondary-foreground dark:text-muted-foreground dark:group-hover:text-foreground"
          >
            {solidIcons.arrowRight}
          </motion.span>
        ) : item.type === "action" ? (
          <div className="flex items-center pl-2 text-muted-foreground group-hover:text-secondary-foreground dark:text-muted-foreground dark:group-hover:text-foreground">
            {solidIcons.arrowRight}
          </div>
        ) : (
          <button
            ...
            className={cn(
              ...
              isChecked ? "bg-primary dark:bg-white" : "bg-border dark:bg-muted",
            )}
          >
            <motion.span
              animate={{
              ...
              className={cn(
                ...
                isChecked ? "bg-white dark:bg-background" : "bg-white dark:bg-muted-foreground",
              )}
            />
          </button>
        )}
      ...
      <AnimatePresence initial={false}>
        {hasSubmenu && isSubmenuOpen && (
          <motion.div
            ...
            className="ml-3 flex w-[calc(100%-0.75rem)] flex-col gap-0.5 overflow-hidden border-l border-border pl-1.5 dark:border-border"
          >
            {submenu.map((submenuItem) => {
              const submenuPath = [...itemPath, submenuItem.id];

              return (
                <FloatingMenuItem
                  key={submenuItem.id}
                  item={submenuItem}
                  itemPath={submenuPath}
                  showIcons={showIcons}
                  isChecked={getItemChecked(submenuPath)}
...
export function FloatingDockMenu({
  ...
  const [menuState, setMenuState] = useState<MenuState>(() => createInitialMenuState(tabs));
  ...
  const toggleSwitch = (tabId: string, itemPath: string[], itemId: string) => {
    const itemStateKey = getItemStateKey(itemPath);
    const nextState = !menuState[tabId]?.[itemStateKey];
    setMenuState((prev) => ({
      ...prev,
      [tabId]: {
        ...prev[tabId],
        [itemStateKey]: nextState,
      },
    }));
      ...
      >
        <LayoutGroup id={`floating-dock-group-${layoutGroupId}`}>
          <motion.div
            ...
            style={
              {
                "--floating-dock-width": isAnyActive ? `${menuWidth}px` : "auto",
              } as React.CSSProperties
            }
            className={cn(
              "relative w-(--floating-dock-width) max-w-[calc(100vw-1rem)] origin-bottom overflow-hidden rounded-[26px]",
              "bg-muted text-foreground dark:bg-background dark:text-foreground",
          ...
          >
            <AnimatePresence initial={false}>
              {currentTab && (
                ...
                >
                  <div className="w-full rounded-[20px] bg-white p-1.5 dark:bg-card/90">
                    <div className="mb-1 flex items-center justify-between px-2.5 py-1.5">
                      <span className="font-mono text-[11px] font-medium tracking-wider text-muted-foreground uppercase dark:text-muted-foreground">
                        ...
                        {currentTab.menuItems.map((item) => {
                          ...
                          return (
                            <FloatingMenuItem
                              ...
                              itemPath={itemPath}
                              showIcons={showIcons}
                              isChecked={!!menuState[currentTab.id]?.[getItemStateKey(itemPath)]}
                              getItemChecked={(path) =>
                                !!menuState[currentTab.id]?.[getItemStateKey(path)]
                              }
                              onToggle={(path, itemId) => toggleSwitch(currentTab.id, path, itemId)}
                              toggleTransition={activeTransition}
                            />
                          );
                        })}
                      </motion.div>
            ...
            <motion.div
              layout
              transition={activeTransition}
              className="flex w-full items-center justify-between rounded-[20px] bg-white p-1 dark:bg-card/90"
            >
              {tabs.map((tab, idx) => {
                  ...
                  <motion.button
                    ...
                    className={cn(
                      ...
                      isActive
                        ...
                        : "text-muted-foreground hover:text-secondary-foreground dark:text-muted-foreground dark:hover:text-foreground",
                    )}
                  >
                    {isActive && (
                      <motion.div
                        ...
                        transition={activeTransition}
                        className="absolute inset-0 rounded-[16px] bg-muted dark:bg-muted/70"
                      />
                    )}
                      ...
                      <AnimatePresence initial={false}>
                        {showLabel && (
                          <motion.span
                            ...
                            className={cn(
                              "overflow-hidden text-xs font-semibold tracking-tight whitespace-nowrap",
                              !isAnyActive && "max-[520px]:hidden",
                            )}
                          >
                            {tab.label}
                          </motion.span>
