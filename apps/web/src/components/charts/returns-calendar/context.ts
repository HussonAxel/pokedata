...
export function useReturnsCalendarModel({
  ...
  const yearSpan =
    yearAnchored && pinned
      ? {
          lo: Math.min(pinned.y, (yearTo ?? pinned).y),
          hi: Math.max(pinned.y, (yearTo ?? pinned).y),
        }
      : null;
  const yearSpanValue = yearSpan
    ? compound(flat.slice(yearSpan.lo * 12, (yearSpan.hi + 1) * 12))
    : 0;
  ...
  const select = (cell: ReturnsCalendarCell) => {
    ...
    else if (pinned && (pinned.m === YEAR) === (cell.m === YEAR))
...
export const ReturnsCalendarContext = createContext<ReturnType<
  typeof useReturnsCalendarModel
> | null>(null);
