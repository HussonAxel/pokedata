...
export function ReturnsCalendarGrid({
  children,
  className,
}: {
  children?: ReactNode;
  ...
  return (
    <div
      ref={gridRef}
      className={cn("returns-calendar-grid relative grid gap-1", className)}
    ...
    >
      ...
      {years.map((year, y) => (
        <div key={year} className="contents">
          <span
            className={cn(
              "flex items-center justify-end pr-1.5 font-mono text-xs tabular-nums transition-colors duration-200",
          ...
          {COLS.map(({ name, m }) => {
            ...
            return (
              ...
              >
                ...
                >
                  <motion.span
                    className="data-chart-cell-surface pointer-events-none absolute inset-0.5 grid place-items-center rounded-xs font-mono text-xs font-medium tabular-nums"
                    style={
                      {
                        "--data-chart-background": tint(v, range, on),
                        "--data-chart-color": "var(--foreground)",
                        "--data-chart-shadow": isEnd ? RING : "none",
                      } as CSSProperties
                    }
                    ...
                    <AnimatePresence>
                      {on && !isEnd ? (
                        <motion.span
                          ...
                          style={{ "--data-chart-ring-color": v >= 0 ? UP : DOWN } as CSSProperties}
          ...
          >
            ...
            >
              <motion.span
                className={cn(
                  "data-chart-cell-content pointer-events-none absolute inset-0.5 grid place-items-center rounded-xs font-mono text-xs font-medium tabular-nums",
                  same(hot, y, YEAR) ? "bg-background" : "bg-foreground/5",
                )}
                // a neutral cell under the colored number: the tint of the same hue ate its contrast
                style={
                  {
                    "--data-chart-color": ink(totals[y] >= 0 ? UP : DOWN),
                ...
                transition={
                  reduce
                    ? { duration: 0 }
                    : { duration: 0.4, ease: EASE_OUT, delay: 0.02 * (y + 13) }
                }
              >
                ...
                <AnimatePresence>
                  ...
                  !(yearSpan ? y === yearSpan.lo || y === yearSpan.hi : same(pinned, y, YEAR)) ? (
                    <motion.span
                      className="data-chart-cell-ring pointer-events-none absolute inset-0"
                      style={
                        { "--data-chart-ring-color": totals[y] >= 0 ? UP : DOWN } as CSSProperties
                      }
                      initial={reduce ? false : { opacity: 0 }}
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
